...
YUI.add("loader-base", function(d) {
  ...
  d.Loader = function(y) {
    ...
    l = t.md5;
    w.context = d;
    w.base = d.Env.meta.base;
    w.comboBase = d.Env.meta.comboBase;
    w.combine = y.base && (y.base.indexOf(w.comboBase.substr(0, 20)) > -1);
    w.maxURLLength = m;
    w.root = d.Env.meta.root;
    w.timeout = 0;
    w.forceMap = {};
    w.allowRollup = true;
    w.filters = {};
    ...
    w.patterns = {};
    w.moduleInfo = {};
    w.groups = d.merge(d.Env.meta.groups);
    w.skin = d.merge(d.Env.meta.skin);
    w.conditions = {};
    w.config = y;
    w._internal = true;
    o = a._renderedMods;
    if (o) {
      r(o, function(A, z) {
        w.moduleInfo[z] = d.merge(A);
      });
      o = a._conditions;
      r(o, function(A, z) {
        w.conditions[z] = d.merge(A);
      });
    } else {
      r(x, w.addModule, w);
    }
    if (!a._renderedMods) {
      a._renderedMods = d.merge(w.moduleInfo);
      a._conditions = d.merge(w.conditions);
    }
    w._inspectPage();
    w._internal = false;
    w._config(y);
  ...
  d.Loader.prototype = {
    ...
    _requires: function(D, C) {
      var z, B, w, E, F, x = this.moduleInfo,
        y = x[D],
        A = x[C];
      if (!y || !A) {
        return false;
      ...
      if (y.ext && y.type == q && !A.ext && A.type == q) {
        return true;
      }
      return false;
    },
    ...
    _addSkin: function(E, C, D) {
      var B, A, w, z = this.moduleInfo,
        x = this.skin,
        y = z[C] && z[C].ext;
      if (C) {
        w = this.formatSkin(E, C);
        if (!z[w]) {
          B = z[C];
          A = B.pkg || C;
          this.addModule({
    ...
    addModule: function(K, R) {
      ...
      if (!K.path && !K.fullpath) {
        K.path = g(R, R, K.type);
      }
      K.ext = ("ext" in K) ? K.ext : (this._internal) ? false : true;
      K.requires = K.requires || [];
      var O = K.submodules,
        N, L, w, H, y, J, x, M, I, F, C, A, z, Q, P, G, B, D = this.conditions,
        E;
      this.moduleInfo[R] = K;
      if (!K.langPack && K.lang) {
        I = j(K.lang);
        for (M = 0; M < I.length; M++) {
          Q = I[M];
          F = this.getLangPackName(Q, R);
          y = this.moduleInfo[F];
          if (!y) {
            y = this._addLangPack(Q, K, F);
          }
        }
      }
      if (O) {
        ...
        for (N in O) {
          if (O.hasOwnProperty(N)) {
            H = O[N];
            H.path = H.path || g(R, N, K.type);
            H.pkg = R;
            H.group = K.group;
            if (H.supersedes) {
              w = w.concat(H.supersedes);
            }
            y = this.addModule(H, N);
            w.push(N);
            if (y.skinnable) {
              K.skinnable = true;
              G = this.skin.overrides;
              if (G && G[N]) {
                for (M = 0; M < G[N].length; M++) {
                  B = this._addSkin(G[N][M], N, R);
                  w.push(B);
                }
              }
              B = this._addSkin(this.skin.defaultSkin, N, R);
              w.push(B);
            }
      ...
      if (J) {
        for (N in J) {
          if (J.hasOwnProperty(N)) {
            x = J[N];
            x.pkg = R;
            x.path = x.path || g(R, N, K.type);
            x.requires = x.requires || [];
            x.group = K.group;
            this.addModule(x, N);
            if (K.skinnable) {
              this._addSkin(this.skin.defaultSkin, N, R);
            }
          }
        }
      }
      if (K.condition) {
        E = K.condition.trigger;
        D[E] = D[E] || {};
        D[E][R] = K.condition;
      }
      if (K.configFn) {
    ...
    require: function(x) {
      var w = (typeof x === "string") ? arguments : x;
      this.dirty = true;
      d.mix(this.required, j.hash(w));
    },
    getRequires: function(M) {
      if (!M || M._parsed) {
        return c;
      }
      var H, E, G, A, z, O, P = M.name,
        y, F, N = n[P] && n[P].details,
        J = [],
        B, K, C, x, L, D = M.lang || M.intl,
        I = this.moduleInfo,
        w = {};
      if (M.temp && N) {
        K = M;
        M = this.addModule(N, P);
        M.group = K.group;
        M.pkg = K.pkg;
        delete M.expanded;
      }
      if (M.expanded && (!M.langCache || M.langCache == this.lang)) {
        return M.expanded;
      }
      B = M.requires;
      C = M.optional;
      ...
      if (M.skinnable) {
        ...
        if (L && L[P]) {
          for (H = 0; H < L[P].length; H++) {
            x = this._addSkin(L[P][H], P);
            J.push(x);
          }
        } else {
          x = this._addSkin(this.skin.defaultSkin, P);
          J.push(x);
        }
      }
      for (H = 0; H < B.length; H++) {
        if (!w[B[H]]) {
          J.push(B[H]);
          w[B[H]] = true;
          E = this.getModule(B[H]);
          if (E) {
            A = this.getRequires(E);
            D = D || (E.expanded_map && (v in E.expanded_map));
      ...
      if (y) {
        r(y, function(R, Q) {
          if (!w[Q]) {
            F = R && ((R.ua && d.UA[R.ua]) || (R.test && R.test(d, B)));
            if (F) {
              w[Q] = true;
              J.push(Q);
              E = this.getModule(Q);
      ...
      M._parsed = false;
      if (D) {
        if (M.lang && !M.langPack && d.Intl) {
          O = d.Intl.lookupBestLang(this.lang || u, M.lang);
          M.langCache = this.lang;
          z = this.getLangPackName(O, P);
          if (z) {
            J.unshift(z);
          }
        }
    ...
    _setup: function() {
      ...
      for (z in C) {
        if (C.hasOwnProperty(z)) {
          w = C[z];
          if (w) {
            w.requires = e.keys(j.hash(w.requires));
    ...
    _onSuccess: function() {
      var w = d.merge(this.skipped),
        x;
      r(w, function(y) {
        delete this.inserted[y];
      }, this);
      this.skipped = {};
      r(this.inserted, function(z, y) {
        d.mix(this.loaded, this.getProvides(y));
      }, this);
      x = this.onSuccess;
      if (x) {
        x.call(this.context, {
      ...
      if (!x) {
        ...
        this._internalCallback = function() {
          ...
          if (this.insertBefore && d.UA.ie) {
            D = d.config.doc.getElementById(this.insertBefore);
            C = D.parentNode;
            A = D.nextSibling;
            C.removeChild(D);
            if (A) {
    ...
    loadNext: function(A) {
      ...
      var H, P, O, M, z, E, B, L, D, G, N, w, C, K, y, F, Q, R, x = this.loadType,
        J = this,
        S = function(T) {
          J.loadNext(T.data);
        },
      ...
      if (this.combine && (!this._combineComplete[x])) {
        F = [];
        this._combining = F;
        H = this.sorted;
        P = H.length;
          ...
          if (K.hasOwnProperty(N)) {
            ...
            for (O = 0; O < P; O++) {
              ...
              if (M && (M.type === x) && (M.combine || !M.ext)) {
                w = (M.root || this.root) + M.path;
                if ((z !== N) && (O < (P - 1)) && ((w.length + z.length) > this.maxURLLength)) {
                  Q.push(this._filter(z));
                  z = N;
                }
                z += w;
                if (O < (P - 1)) {
      ...
      P = H.length;
      for (O = 0; O < P; O = O + 1) {
        if (H[O] in this.inserted) {
          continue;
        }
        if (H[O] === this._loading) {
          return;
        }
        M = this.getModule(H[O]);
        if (!M) {
          B = "Undefined module " + H[O] + " skipped";
          this.skipped[H[O]] = true;
          continue;
        }
        D = (M.group && this.groups[M.group]) || f;
        if (!x || x === M.type) {
          this._loading = H[O];
          if (M.type === q) {
            E = d.Get.css;
          ...
          z = (M.fullpath) ? this._filter(M.fullpath, H[O]) : this._url(M.path, H[O], D.base || M.base);
          E(z, {
            data: H[O],
            onSuccess: S,
            insertBefore: this.insertBefore,
        ...
        if (o.hasOwnProperty(k)) {
          if (!b[k] && ((!this.loaded[k]) || this.forceMap[k])) {
            ...
            for (h = 0; h < p.length; h = h + 1) {
              n = f[p[h]];
              if (this.loaded[p[h]] && !this.forceMap[p[h]]) {
                e = false;
                break;
              } else {
                if (b[p[h]] && g.type == n.type) {
                  l++;
                  e = (l >= g.rollup);
                  if (e) {
                    break;
...
YUI.add("loader-yui3", function(a) {
  YUI.Env[a.version].modules = YUI.Env[a.version].modules || {
    ...
    "editor": {
      "submodules": {
        ...
        "editor-base": {
          "requires": ["base", "frame", "node", "exec-command", "selection", "editor-para"]
        },
        "editor-bidi": {
          "requires": ["editor-base"]
        },
  ...
  YUI.Env[a.version].md5 = "cb1db98536fb15d179c178aab4be9340";
}, "3.2.0", {
  requires: ["loader-base"]
});
YUI.add("loader", function(a) {}, "3.2.0", {
