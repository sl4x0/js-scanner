...
async def test_progressive_timeout_increases(mock_async_session, strategies_config):
    ...
    strategies_config['active']['max_retries'] = 3
    ...
    async def mock_get(*args, **kwargs):
        nonlocal call_count
        timeouts_used.append(kwargs.get('timeout', 'no-timeout'))
        call_count += 1
        if call_count < 3:
    ...
    # Verify progressive timeout was used
    assert len(timeouts_used) == 3
    assert timeouts_used[0] == 10  # First attempt
    assert timeouts_used[1] == 20  # Second attempt (2x)
    assert timeouts_used[2] == 30  # Third attempt (3x)
...
async def test_browser_fallback_on_403(mock_async_session, mock_browser_manager, strategies_config):
    """Test 403 response triggers Playwright fallback"""
    noise_filter = Mock()
